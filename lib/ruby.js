...
const {ひらがなへ, カタカナへ} = require('./func')
...
module.exports = class ルビ {
  constructor(元のテキスト = '', kuromoji解析json = [], オプション) {
    ...
    this.option = オプション
    if (this.option.グループ === '--html' && this.option.use_span) {
      this.option.始まり = '<span data-ruby = \"'
    } else if(this.option.グループ === '--html') {
  ...
  漢字仮名交じりルビ(漢字仮名, ルビ) {
    ...
    if (this.option.granularity === 2) {
      if (this.option.katakana) {
        return 始まり + ルビ + 左括弧 + 漢字仮名 + 右括弧
      } else if (this.option.use-span){
	  return 始まり + 漢字仮名 + 左括弧 + ひらがなへ(ルビ) + 右括弧  
      } else{
          return 始まり + ひらがなへ(ルビ) + 左括弧 + 漢字仮名 + 右括弧 //temporary inverted
      }
    }
...
<<<<<<< HEAD
      if (カタカナなの) {
        return 語頭 + 始まり + 漢字かな + 左括弧 + 語頭語尾除いたルビ + 右括弧 + 語尾
      } else {
          return 語頭 + 始まり + ひらがなへ(語頭語尾除いたルビ) + 左括弧 + 漢字かな + 右括弧 + 語尾
      }
=======
      if (カタカナなの) return 語頭 + 始まり + 漢字かな + 左括弧 + 語頭語尾除いたルビ + 右括弧 + 語尾
      return 語頭 + 始まり + 漢字かな + 左括弧 + ひらがなへ(語頭語尾除いたルビ) + 右括弧 + 語尾
>>>>>>> d5c3c4fe807701cf23f9babd3ce5e15f67700c38
    }
    ...
    // inverted
    return 語頭 + 始まり + 前ルビ + 左括弧 + 前漢字 + 右括弧 +
      漢字かな.split(真ん中かな表現)[1] +
<<<<<<< HEAD
      始まり + 後ルビ + 左括弧 + 後漢字 + 右括弧 + 語尾
